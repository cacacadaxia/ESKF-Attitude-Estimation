...
%     q(t,:) = madgwickAHRS(Data(t,27:29), Data(t,9:11), Data(t,15:17),q(t-1, :),Beta,SamplePeriod);	%梯度下降法  
   ...
   [roll,pitch,yaw] = quattoeuler(q(t,:));
   oula_ahrs(t,:) = [roll,pitch,yaw];
end
clear q;
...
subplot(3,1,1)
plot(oula(:,1) - rollRef);
hold on;plot(oula_ahrs(:,1) - rollRef);
subplot(3,1,2)
plot(oula(:,2)- pitchRef);
hold on;plot(oula_ahrs(:,2) - pitchRef);
subplot(3,1,3)
plot(oula(:,3)- yawRef);
hold on;plot(oula_ahrs(:,3) - yawRef);

error = oula- [rollRef , pitchRef , yawRef];
